...
const handleModifyPersonalInformationClick = async (newValue: string | null, field: string) => {
    ...
    if (newValue && user && user.accessToken) {
        try {
            const response = await putCallAPIWithToken(API_FOR_PERSONAL_INFORMATION_MODIFY, { [field]: newValue }, user.accessToken);
            if (response.success) {
