...
const Navbar = () => {
  ...
  const quantity = shoppingCart?.items?.reduce((acc, cartItem) => acc + cartItem.quantity, 0) || 0;
      ...
      <div className="navbar bg-base-100 shadow rounded-2xl mt-8 w-5/6 md:w-4/6 lg:w-3/5 mx-auto">
        <div className="flex-1">
          <Link href="/" className="btn btn-ghost text-xl flex items-center">
            <Image src="/favicon.ico" alt="Airneis" width={40} height={40} className="me-2" />
            <span>Airneis</span>
          </Link>
            ...
            <div className="absolute top-full right-0 mr-8 bg-white shadow-lg rounded-lg p-4 flex flex-col items-start sm:hidden z-50">
              ...
              {isLoggedIn ? (
                <>
                  ...
                  <button onClick={handleClickForLogout}><Link href="/" className="hover:text-gray-400 hover:font-bold">Déconnexion</Link></button>
                </>
              ) : (
                <>
            ...
            <div className="dropdown dropdown-end">
              ...
              <div tabIndex={0} className="mt-3 z-[1] card card-compact dropdown-content w-52 bg-base-100 shadow">
                <div className="card-body">
                  <span className="font-bold text-lg">{quantity} {quantity > 1 ? 'articles' : 'article'}</span>
                  <span className="font-semibold">{shoppingCart?.total ? `Total : ${shoppingCart.total} €` : ''}</span>
