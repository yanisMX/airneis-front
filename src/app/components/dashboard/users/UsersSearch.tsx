import { UserQuery } from "@/app/interfaces/interfaces";
import { useEffect, useRef } from "react";

export default function UsersSearch({ filters, setFilters, areFiltersActive }: { filters: UserQuery, setFilters: (filters: UserQuery) => void, areFiltersActive: boolean }) {
  ...
  return <form className="flex items-center space-x-2" onSubmit={(e) => { e.preventDefault() }}>
    ...
    <button className={"btn btn-sm p-2 " + (areFiltersActive ? "btn-primary" : "btn-outline opacity-40")} title="Filtrer" onClick={() => {
      const element = document.getElementById("users-filters");
