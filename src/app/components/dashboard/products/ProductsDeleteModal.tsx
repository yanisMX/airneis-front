...
export default function ProductsDeleteModal({ id, product, fetchProducts }: { id: string, product: Product, fetchProducts: () => void }) {
  const { user } = useAuth();
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      fetchProducts();
      toast.success(() => <span>Le produit <b>{product.name}</b> <span className="opacity-50">#{product.id}</span> a bien été supprimé.</span>);
    } catch (error) {
      console.error(error);
  ...
  return <>
    <dialog id={id} className="modal modal-bottom sm:modal-middle">
      <div className="modal-box font-normal">
        ...
        <p className="pb-8">Êtes-vous sur de vouloir supprimer <b>{product.name}</b> ?</p>
