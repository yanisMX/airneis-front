...
export default function MaterialsSearch({ filters, setFilters, areFiltersActive }: { filters: MaterialFilters, setFilters: (filters: MaterialFilters) => void, areFiltersActive: boolean }) {
  ...
  return <form className="flex items-center space-x-2" onSubmit={(e) => { e.preventDefault() }}>
    ...
    <button className={"btn btn-sm p-2 " + (areFiltersActive ? "btn-primary" : "btn-outline opacity-40")} title="Filtrer" onClick={() => {
      const element = document.getElementById("materials-filters");
