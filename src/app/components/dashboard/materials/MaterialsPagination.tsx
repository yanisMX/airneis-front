import { MaterialFilters, MaterialPagination } from "@/app/interfaces/interfaces";

export default function MaterialsPagination({ pagination, setPagination, filters, setFilters }: { pagination: MaterialPagination, setPagination: (pagination: MaterialPagination) => void, filters: MaterialFilters, setFilters: (filters: MaterialFilters) => void }) {
  ...
  const renderLimitOptions = () => (
    [5, 10, 15, 20].map(limit => (
      <li key={limit} onClick={() => setFilters({ ...filters, page: 1, limit })} className="menu-item">
        <button className={`btn btn-xs ${pagination.limit === limit ? "btn-primary" : "btn-ghost"}`}>{limit} matériaux</button>
      </li>
    ))
  );
  ...
  return (
    <div className="flex items-center justify-end sm:space-x-12">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center flex-1">
        <div className="text-sm ms-2">{pagination.materialCount} matériaux, {pagination.totalPages} pages</div>
        <div className="hidden md:block">
          <div className="dropdown dropdown-top me-1">
            <div tabIndex={0} role="button" className="btn btn-xs m-1">
              <i className="fa-solid fa-chevron-up"></i>{pagination.limit} matériaux
            </div>
